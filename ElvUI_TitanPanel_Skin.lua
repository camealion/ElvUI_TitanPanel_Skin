...
-- Options	Window
	...
	-- Screenshot Window	
		...
		TP_Skin_PreviewFrame:SetTemplate('Transparent')
				...
				previewZoom:SetTexture([[Interface\AddOns\ElvUI_TitanPanel_Skin\media\Bar3and4.tga]])
				previewZoom:Show()
...
-- Enable the bars
		...
		tpskin:SetScript("OnUpdate", function()
			if Titan_Bar__Display_Bar2 and Titan_Bar__Display_AuxBar2 then -- Do they really exist!
				tpskin_checkbox:SetChecked(EnableSpecialBars)
			else
	...
	if EnableSpecialBars == true then
		...
		local y = Titan_Bar__Display_Bar:GetHeight() + Titan_Bar__Display_Bar2:GetHeight()/2 + 1
				...
				if Bar2and3Width == nil then 
					...
					Titan_Bar__Display_Bar2:SetWidth(Bar2and3Width)
				end
				...
				if Bar2and3Width == nil then 
					...
					Titan_Bar__Display_AuxBar2:SetWidth(Bar2and3Width)
				end
